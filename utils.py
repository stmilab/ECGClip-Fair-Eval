...
def plot_features(model, vdl, epoch, num_classes, num_feats, batch_size, name=None):
    ...
    for i in range(num_classes):
        plt.scatter(
            x_feats[vallabels[:num_samples] == i, 1],
            x_feats[vallabels[:num_samples] == i, 0],
        )

    plt.legend([str(i) for i in range(num_classes)])
    plt.show()
    plt.savefig(f"./FIGS/{name}_embedding_{epoch}.jpeg")
