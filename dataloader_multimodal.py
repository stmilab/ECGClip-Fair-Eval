...
class ECGDataset(Dataset):
    def __init__(
        self,
        root="./",
        metadata_dir=f"/ssd-shared/mimiciv-ecg-echonotes/physionet.org/files/mimic-iv-ecg/1.0/processed/metadatav3_w_icd.csv",
        npy_file_dir="",
        split="Train",
        from_numpy=True,
        ecg_transform=None,
        ...
        if self.from_numpy:
            ...
            self.meta_data = self.meta_data.dropna(subset=["report_0"])
            ...
            self.meta_data = self.meta_data.iloc[:50000].reset_index(drop=True)
    ...
    def get_data(self):
        for idx in tqdm(range(len(self))):
            ...
            self.report[idx] = self.meta_data.iloc[idx]["report_0"]
